...
function gebeyashoptheme_homepage_category1_customizer($wp_customize) {
    ...
    $wp_customize->add_section('gebeyashoptheme_home_category1', array(
        'title' => __('Homepage Category 1', 'gebeyashoptheme'),
        'priority' => 37,
    ));

    // TITLE FIELD
    $wp_customize->add_setting('gebeyashoptheme_home_category1_title', array(
        'default' => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));

    $wp_customize->add_control('gebeyashoptheme_home_category1_title', array(
        'label' => __('Section Title', 'gebeyashoptheme'),
        'section' => 'gebeyashoptheme_home_category1',
        'type' => 'text',
    ));

    // CATEGORY SETTING
    $wp_customize->add_setting('gebeyashoptheme_home_category1_categories', array(
        'default' => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    ...
    class Gebeya_Category1_Control extends WP_Customize_Control {
        ...
        public function render_content() {
            ...
            <input type="hidden" <?php $this->link(); ?> id="category1Output">

            <!-- JS -->
            <script>
                (function () {
                    const checkboxes = document.querySelectorAll('#category1List input[type="checkbox"]');
                    const output = document.getElementById('category1Output');
                    ...
                    // Search
                    document.getElementById('category1Search').addEventListener('keyup', function () {
                        let val = this.value.toLowerCase();
                        document.querySelectorAll('#category1List label').forEach(label => {
                            label.style.display = label.textContent.toLowerCase().includes(val) ? 'block' : 'none';
                        });
                    });
    ...
    // CONTROL
    $wp_customize->add_control(new Gebeya_Category1_Control(
        $wp_customize,
        'gebeyashoptheme_home_category1_control',
        array(
            'section' => 'gebeyashoptheme_home_category1',
            'settings' => 'gebeyashoptheme_home_category1_categories',
        )
    ));
}
